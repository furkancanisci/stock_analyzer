...
app = Flask(__name__)
CORS(app) # Tüm kökenlerden gelen isteklere izin ver (Geliştirme için iyi, üretim için kısıtlanmalı)
...
def load_open_positions():
    ...
    if os.path.exists(POSITIONS_FILE):
        try:
            ...
            log_message(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Açık pozisyonlar yüklendi.")
        except Exception as e:
            log_message(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Pozisyonlar yüklenirken hata oluştu: {e}")
            open_positions = {}
    else:
        open_positions = {}
...
def add_technical_indicators(df):
    ...
    try:
        ...
        if not all(col in df.columns for col in required_cols):
            ...
            log_message(f"UYARI: Teknik göstergeler için eksik temel sütunlar: {missing_cols}", level="ERROR")
        ...
        if close_series.ndim != 1 or close_series.empty or len(close_series) < min_bars_for_ta: 
            log_message(f"HATA: 'Close' serisi 1D değil ({close_series.ndim}), boş veya yetersiz bar ({len(close_series)}).", level="ERROR")
            return pd.DataFrame()
        if high_series.ndim != 1 or high_series.empty or len(high_series) < min_bars_for_ta:
            log_message(f"HATA: 'High' serisi 1D değil ({high_series.ndim}), boş veya yetersiz bar ({len(high_series)}).", level="ERROR")
            return pd.DataFrame()
        if low_series.ndim != 1 or low_series.empty or len(low_series) < min_bars_for_ta:
            log_message(f"HATA: 'Low' serisi 1D değil ({low_series.ndim}), boş veya yetersiz bar ({len(low_series)}).", level="ERROR")
            return pd.DataFrame()
        if volume_series.ndim != 1 or volume_series.empty or len(volume_series) < min_bars_for_ta:
            log_message(f"HATA: 'Volume' serisi 1D değil ({volume_series.ndim}), boş veya yetersiz bar ({len(volume_series)}).", level="ERROR")
            return pd.DataFrame()
        ...
        df['PSAR'] = ta.trend.PSARIndicator(high_series, low_series, close_series).psar() 
        ...
        if len(df) < 50: # Sinyal kontrolü için en az 50 temiz bar olsun
             log_message(f"UYARI: add_technical_indicators sonrası yetersiz bar sayısı ({len(df)}).", level="ERROR")
             return pd.DataFrame()

        return df
    except Exception as e:
        log_message(f"CRITICAL HATA: Teknik göstergeler eklenirken beklenmedik bir hata oluştu: {e}", level="CRITICAL")
...
def is_bullish_engulfing(df_slice):
    ...
    if any(pd.isna(current_candle[col]) for col in ['Close', 'Open']) or \
       any(pd.isna(prev_candle[col]) for col in ['Close', 'Open']):
        return False

    prev_is_bearish = prev_candle['Close'].item() < prev_candle['Open'].item() # Önceki mum düşüş mumu mu?
    current_is_bullish = current_candle['Close'].item() > current_candle['Open'].item() # Şimdiki mum yükseliş mumu mu?
    
    engulfs_open = current_candle['Open'].item() < prev_candle['Close'].item() # Şimdiki mumun açılışı önceki mumun kapanışından düşük mü?
    engulfs_close = current_candle['Close'].item() > prev_candle['Open'].item() # Şimdiki mumun kapanışı önceki mumun açılışından yüksek mi?
    ...
    if prev_body_size == 0: return False # Sıfır bölme hatasını önle
    body_size_condition = current_body_size > (prev_body_size * 1.2) # Şimdiki mumun gövdesi önceki mumdan %20 daha büyük mü?
...
def is_bearish_engulfing(df_slice):
    ...
    prev_is_bullish = prev_candle['Close'].item() > prev_candle['Open'].item() # Önceki mum yükseliş mumu mu?
    current_is_bearish = current_candle['Close'].item() < current_candle['Open'].item() # Şimdiki mum düşüş mumu mu?
    
    engulfs_open = current_candle['Open'].item() > prev_candle['Close'].item() # Şimdiki mumun açılışı önceki mumun kapanışından yüksek mi?
    engulfs_close = current_candle['Close'].item() < prev_candle['Open'].item() # Şimdiki mumun kapanışı önceki mumun açılışından düşük mü?
...
def analyze_stock_for_api(ticker):
    ...
    stock_status = {
        ...
        "lowPrice": 0.0,
        "volume": 0,
        "RSI": 0.0, "MACD": 0.0, "MACD_Signal": 0.0, "SMA_20": 0.0, "SMA_50": 0.0, "PSAR": 0.0,
        "buy_signal": False, "buy_details": "",
        "sell_signal": False, "sell_details": "",
    ...
    if data_1h.empty or data_1d.empty or \
       len(data_1h) < required_min_bars_1h or len(data_1d) < required_min_bars_1d:
        log_message(f"UYARI: {ticker} için sinyal kontrolü için yeterli veri yok. 1h bar: {len(data_1h)}, 1d bar: {len(data_1d)}", level="WARN")
        stock_status["status_text"] = "Yetersiz Veri"
        stock_status["status_class"] = "text-red-500"
        return {"status": "no_data", "message": "Yeterli veri alınamadı veya hesaplama için yetersiz bar.", "data": stock_status}
    ...
    if data_1h_processed.empty or data_1d_processed.empty:
        log_message(f"HATA: {ticker} için teknik göstergeler hesaplanamadı.", level="ERROR")
        stock_status["status_text"] = "Gösterge Hatası"
        stock_status["status_class"] = "text-red-500"
        return {"status": "error_processing", "message": "Teknik göstergeler hesaplanamadı.", "data": stock_status}
    ...
    try:
        ...
        indicators_to_assign = ["RSI", "MACD", "MACD_Signal", "SMA_20", "SMA_50", "PSAR"]
        ...
        if not pd.isna(prev_1h_data['Close'].item()) and prev_1h_data['Close'].item() != 0:
            stock_status["change"] = round(((latest_1h_data['Close'].item() - prev_1h_data['Close'].item()) / prev_1h_data['Close'].item()) * 100, 2)
        else:
        ...
        if ticker not in open_positions: 
            ...
            rsi_1h_condition = not pd.isna(latest_1h_data['RSI'].item()) and latest_1h_data['RSI'].item() > 55 
            
            sma_crossover_1h_condition = (
                not pd.isna(prev_1h_data['SMA_20'].item()) and not pd.isna(prev_1h_data['SMA_50'].item()) and
                not pd.isna(latest_1h_data['SMA_20'].item()) and not pd.isna(latest_1h_data['SMA_50'].item()) and
                (prev_1h_data['SMA_20'].item() < prev_1h_data['SMA_50'].item() and
                 latest_1h_data['SMA_20'].item() > latest_1h_data['SMA_50'].item())
            )

            macd_1h_condition = (
                not pd.isna(latest_1h_data['MACD'].item()) and not pd.isna(latest_1h_data['MACD_Signal'].item()) and
                latest_1h_data['MACD'].item() > 0 and 
                latest_1h_data['MACD'].item() > latest_1h_data['MACD_Signal'].item()
            )

            daily_trend_condition = (
                not pd.isna(latest_1d_data['SMA_50'].item()) and not pd.isna(latest_1d_data['SMA_200'].item()) and
                not pd.isna(latest_1d_data['MACD'].item()) and
                latest_1d_data['SMA_50'].item() > latest_1d_data['SMA_200'].item() and
            ...
            if pd.isna(avg_volume_1h) or pd.isna(max_volume_1h_50_period):
                ...
                volume_condition = (
                    not pd.isna(latest_1h_data['Volume'].item()) and # Hacim de NaN olmamalı
                    latest_1h_data['Volume'].item() > (avg_volume_1h * 1.2) and 
            ...
            if (rsi_1h_condition and 
                ...
                volume_condition): 
                buy_signal = True
                buy_details = "BUY: RSI>55, SMA_CO, MACD_POS, DailyTrend, HighVol (Gevşetilmiş)"
                enter_position(ticker, latest_1h_data['Close'].item(), "BUY", buy_details)
        
        # DÜŞÜŞ (SATIŞ/KAR ALMA) SİNYALİ KONTROLÜ
        if ticker in open_positions:
            psar_condition = not pd.isna(latest_1h_data['PSAR'].item()) and latest_1h_data['Close'].item() < latest_1h_data['PSAR'].item()
            close_below_sma20 = not pd.isna(latest_1h_data['SMA_20'].item()) and latest_1h_data['Close'].item() < latest_1h_data['SMA_20'].item()
            
            macd_negative_or_crossover = (
                not pd.isna(latest_1h_data['MACD'].item()) and not pd.isna(latest_1h_data['MACD_Signal'].item()) and
                not pd.isna(prev_1h_data['MACD'].item()) and not pd.isna(prev_1h_data['MACD_Signal'].item()) and
                (latest_1h_data['MACD'].item() < 0 or 
            ...
            if psar_condition or close_below_sma20 or macd_negative_or_crossover:
                ...
                exit_position(ticker, latest_1h_data['Close'].item(), "SELL", sell_details)
        ...
        log_message(f"HATA: {ticker} için sinyal işlenirken veya API yanıtı hazırlanırken hata oluştu: {e}", level="ERROR")
        ...
        return {"status": "error_processing", "message": f"İşlem hatası: {e}", "data": stock_status}
...
def analyze_stocks_api():
    stocks_to_monitor = ["AAPL", "MSFT", "GOOGL", "AMZN", "NVDA", "TSLA", "META", "NFLX", "JPM", "XOM", "JNJ", "WMT"]
    results = []
    ...
    for ticker in stocks_to_monitor:
        ...
        if analysis_result["status"] == "ok" and analysis_result["data"]["buy_signal"] and buy_signal_stock is None:
